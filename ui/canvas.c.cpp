...
static void drawfunc_clear(pixel *ptr, int pitch, void *object, int x, int y)
{
	...
	for (i = 0; i<CANVAS_BLOCK_HEIGHT; i++) 
	{
		...
		//else
		//Sleep(100);
		memset(ptr,a, CANVAS_BLOCK_WIDTH*sizeof(pixel));
...
static void canvas_init(struct canvas *c, pixel *ptr, draw_func flip, void *flip_object)
{
	...
	for (i = 0; i<h; i += CANVAS_BLOCK_HEIGHT)
	{
		for (j = 0; j<w; j += CANVAS_BLOCK_WIDTH)
		{
			...
			block->pipeline[0].object = flip_object;//*gdi_object
...
void canvas_draw(struct canvas *c, draw_func func, void *object, int x, int y)//func:draw_box   object:box *b
{
...
void canvas_update(struct canvas *c)
{
	...
	for (i = c->block_w*c->block_h; i>0; --i, ++blk)
	{
		...
		{
			pixel *ptr = blk->ptr;
			struct pipe_node *node = NULL;
 			drawfunc_clear(ptr, pitch, 0, 0, 0);
		
			for (j = 1; j<blk->total_object; j++)
			{
				node = &blk->pipeline[j];
	...
	struct pipe_node *node = NULL;
	node = &(c->block->pipeline[0]);
		//node = &blk->pipeline[0];
	node->func(c->block->ptr, pitch, node->object, node->x, node->y);
		//Sleep(100);
}
